...
#include "image.h"
//Here I will read in the padded image along with the pre-made
//Filter
//I will execute in this order: ./executable paddedfile.pgm filter.pgm output.pgm
...
ImageType image2(rows, cols, bytes);
...
///////////////////////////  Compute the product G(u,v) = F(u,v)H(u,v) //////////////////////
float maskValue = 0.0;
...
for(int i = 0; i < N; i++)
{
  ...
  {
    int value;
    image2.getPixelVal(i,j,value);
    maskValue = (float)value;
    maskValue /= 255.0;
    /////////////////////////////// This part to isolate Frequency in image.
		//Uncomment the line below to EXTRACT FREQUENCY!!!!
    //maskValue = 1 - maskValue;

    ////////////////////////////////
    image_real[i][j] = image_real[i][j] * maskValue;
    image_imag[i][j] = image_imag[i][j] * maskValue;
  }
}
...
for (int i = 0; i < rows; ++i)
{
  ...
  for (int j = 0; j < cols; ++j)
  {
    image_real[i+1][j+1] *= pow(-1, ((i+1)+(j+1))); //this uncenters the image
    image_out[i][j] = image_real[i+1][j+1];
  }
}
